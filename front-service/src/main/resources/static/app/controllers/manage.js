angular.module('jwtApp')
    ...
    $scope.deleteProduct = function (productId) {
        $http.delete('http://localhost:9099/api/v1/products/' + productId)
    ...
    $scope.changeCost = function (product, delta) {
        ...
        $http.put('http://localhost:9099/api/v1/products', body)
            .then(function (response) {
    ...
    $scope.addProduct = function (form) {
        ...
        $http.post('http://localhost:9099/api/v1/products', body)
            .then(function (response) {
                $scope.loadProducts($scope.page);
                $scope.data = {};
            });
    ...
    $scope.loadProducts = function (page) {
        $scope.page = page;
        $http({
            url: 'http://localhost:9099/api/v1/products',
            method: 'GET',
            params: {
